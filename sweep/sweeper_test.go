...
// TestSweepPendingInputsReleasesOnFailure verifies that when a set
// fails to be swept after wallet inputs were leased, the sweeper
// releases all the leased outputs so they aren't left locked.
func TestSweepPendingInputsReleasesOnFailure(t *testing.T) {
	t.Parallel()
	...
	// GenSweepScript is the easiest place to inject a failure into
	// the sweep path: it runs early in s.sweep, so the rest of the
	// set's methods are never reached.
	genErr := errors.New("gen sweep script failed")
	s := New(&UtxoSweeperConfig{
		Wallet:     wallet,
		Aggregator: aggregator,
		Publisher:  publisher,
		GenSweepScript: func() fn.Result[lnwallet.AddrWithKey] {
			return fn.Err[lnwallet.AddrWithKey](genErr)
		},
		NoDeadlineConfTarget: uint32(DefaultDeadlineDelta),
	})
	s.currentHeight = testHeight

	// The wallet UTXO that AddWalletInputs claimed before the
	// sweep failed.
	...
	set.On("NeedWalletInput").Return(true).Once()
	set.On(
		"AddWalletInputs", wallet, mock.Anything,
	).Return([]wire.OutPoint{walletOP}, nil).Once()

	// The coin select lock is taken for the only set.
	wallet.On(
		"WithCoinSelectLock", mock.Anything,
	).Return(nil).Once()

	// The critical assertion: when the sweep fails, ReleaseOutput
	// must be called for each previously leased outpoint.
	wallet.On(
		"ReleaseOutput", mock.Anything, walletOP,
	).Return(nil).Once()

	pis := make(InputsMap)
	aggregator.On("ClusterInputs", pis).Return([]InputSet{set})
...
func TestSweepPendingInputsExcludesUsedUtxos(t *testing.T) {
	...
	// First set: claims walletOP, sees an EMPTY exclusion set.
	set1 := &MockInputSet{}
	...
	// Second set: must see walletOP in its exclusion set. This is
	// the property under test; mock.MatchedBy inspects the set
	// argument and asserts membership.
	set2 := &MockInputSet{}
